...
def match_template(im_search, im_source):
    ...
    if len(im_source.shape) == 3:  # 如果是三通道，转换为灰度图
        im_source = cv2.cvtColor(im_source, cv2.COLOR_BGR2GRAY)
    else:
        raise ValueError("im_source的通道数不正确")

    if len(im_search.shape) == 3:  # 如果是三通道，转换为灰度图
        im_search = cv2.cvtColor(im_search, cv2.COLOR_BGR2GRAY)
    else:
        raise ValueError("im_search的通道数不正确")
    # 初始化创建
...
def find_template_center(im_source, im_search, scale_range=(0.9, 1.1), scale_step=0.1):
    # 将原图转换为灰度图
    if len(im_source.shape) == 3:
        source_gray = cv2.cvtColor(im_source, cv2.COLOR_BGR2GRAY)
    else:
        source_gray = im_source
    ...
    for scale in np.arange(scale_range[0], scale_range[1], scale_step):
        ...
        if max_val > best_val:
            ...
            best_center = (best_match[0] + w // 2, best_match[1] + h // 2)
...
if __name__ == '__main__':
    # 模版
    img1 = cv2.imread("/Users/hebert/PycharmProjects/dnfm-yolo-tutorial-macos/template/再次挑战.png", 0)
    # 原图
    img2 = cv2.imread("/Users/hebert/Desktop/Snipaste_2024-08-07_14-50-46.png")
    # s = time.time()
    # crop = (2200, 100, 304, 304)
    ...
    print(find_template_center(img2, img1))
